...
my $maco = Lingua::FreeLing::MorphAnalyzer->new("es",
                                                AffixAnalysis => 1,
...
# defined
ok($maco);
isa_ok($maco => 'Lingua::FreeLing::MorphAnalyzer');
ok(exists($maco->{maco}));
ok(exists($maco->{prefix}));
ok(exists($maco->{maco_options}));
isa_ok($maco->{maco}         => 'Lingua::FreeLing::Bindings::maco');
isa_ok($maco->{maco_options} => 'Lingua::FreeLing::Bindings::maco_options');
...
my $tok = Lingua::FreeLing::Tokenizer->new('es');
my $spl = Lingua::FreeLing::Splitter->new('es');
...
isa_ok($analysis->[0] => "Lingua::FreeLing::Word::Analysis");
