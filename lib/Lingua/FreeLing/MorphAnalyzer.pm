package Lingua::FreeLing::MorphAnalyzer;
...
(C<Lingua::FreeLing> will search for the data file) or the full or
...
sub new {
    ...
    my $dir;
    if ($lang =~ /^[a-z][a-z]$/i) {
        $dir = Lingua::FreeLing::_search_language_dir($lang);
    ...
    my $self = bless {
                      prefix => $dir,
                      maco_options => Lingua::FreeLing::Bindings::maco_options->new($lang),
                     } => $class;
    ...
    $self->{maco} = Lingua::FreeLing::Bindings::maco->new($self->{maco_options});
    return $self;
}
